...
class LiteLunchLounge{
	...
	public static void showMenu(){
		...
		System.out.println("---------------------------Items----------------------------Balance------------");
	...
	public static int coffeeBalance(int q){
		...
		if(CoffeeBalance >= q){
	    	...
	    	System.out.println("You  order *" + qtyCoffee + " Soft Drink.");
	    	...
	    	if(CoffeeBalance == 0){
	    		...
	    		CoffeeBalance = 0;
	    		qtyCoffee = qtyCoffee + q;
	    		System.out.println("You Just can order *" + qtyCoffee + "Coffee.Do you want buy? Y/N");	
	    	}	
	    }
	...
	public static int softDrinkBalance(int q){
		...
		if(SoftDrinkBalance >= q){
	    	...
	    	if(SoftDrinkBalance == 0){
	    	   	 ...
	    	   	 System.out.println("You  just can order *"+ qtySoftDrink +" Soft Drink.Do you want buy ? Y/N ");
	    	   	 char ans=keybord.next().charAt(0);
	    	   	 if(ans == 'y' || ans == 'Y'){
	    	   	  SoftDrinkBalance = 0;
	    	      qtySoftDrink = qtySoftDrink + q;
	    	     }
	...
	public static int dessertBalance(int q){
		...
		if(DessertBalance >= q){
	    	...
	    	if(DessertBalance == 0){
	    	     System.out.println("Sorry. This product is out Of Stock."); 
	    	   	 DessertBalance=0;
	    	   	 qtyDessert=qtyDessert;	 
	    	   }    
	    	 else{	
	    	   	 System.out.println("You  just can order *" + qtyDessert + " Dessert.Do you want buy ? Y/N ");
	    	   	 char ans = keybord.next().charAt(0);
	    	   	 if(ans == 'y' || ans == 'Y'){
	    	   	 	DessertBalance = 0;
	    	      	qtyDessert = qtyDessert + q;
	...
	public static int starterBalance(int q){
		...
		if(StarterBalance >= q){
	    	...
	    	if(StarterBalance == 0){
	    		...
	    		System.out.println("You  just can order *" + qtyStarter + "Starter.Do you want buy ? Y/N ");
	    		char ans=keybord.next().charAt(0);
	    	   	if(ans == 'y'||ans == 'Y'){
	    	   	  StarterBalance = 0;
	    	      qtyStarter = qtyStarter + q;
		...
		if(MainCourseBalance >= q){
			...
			System.out.println("You  order *" + qtyMainCourse + " Main Course.");	
	    	   ...
	    	   if(MainCourseBalance == 0){
	    	   	 System.out.println("Sorry. This product is out of Stock."); 
	    	   	 MainCourseBalance = 0;
	    	   	 qtyMainCourse = qtyMainCourse;
	    	   }    
	    	   else{
	      	   	  System.out.println("You  just can order *"+qtyMainCourse+" Main Course.Do you want buy ? Y/N ");
	    	   	  char ans = keybord.next().charAt(0);
	    	   	  if(ans == 'y' || ans == 'Y'){
	    	   		MainCourseBalance = 0;
	    	      	qtyMainCourse = qtyMainCourse + q;
		...
		if(MainCourseDessertBalance >= q){
	    	...
	    	if(MainCourseDessertBalance == 0)
	    	{
	    	   ...
	    	   else 
	    	   {
	    	   	 ...
	    	   	 System.out.println("You  just can order *"+qtyDessert+" Main Course + Dessert.Do you want buy ? Y/N ");
	    	   	  char ans=keybord.next().charAt(0);
	    	   	  if(ans=='y'||ans=='Y')
	    	   	  {
	    	   		MainCourseDessertBalance=0;
		...
		public static int mainCourseStarterBalance(int q)
	{
		...
		if(	MainCourseStarterBalance>=q)
		{
			...
			System.out.println("You  order *"+qtyStarter+" Main Course + Starter.");
			
		}	
	    else 
	    {
	    	    
	    	   q=	MainCourseStarterBalance-0;
	    	   ...
	    	   else 
	    	   {
	    	   	 ...
	    	   	 System.out.println("You  just can order *"+qtyStarter+" Main Course + Starter.Do you want buy ? Y/N ");
	    	   	  char ans=keybord.next().charAt(0);
	    	   	  if(ans=='y'||ans=='Y')
	    	   	  {
	    	   		MainCourseStarterBalance=0;
		...
		public static int comboBalance(int q)
	{
	    ...
	    else 
	    {
	    	   ...
	    	   else 
	    	   {
	    	   	 ...
	    	   	 System.out.println("You  just can order *"+qtyCombo+" Combo (Main Course + Dessert + Starter).Do you want buy ? Y/N ");
	    	   	  char ans=keybord.next().charAt(0);
	    	   	  if(ans=='y'||ans=='Y')
	    	   	  {
	    	   		ComboBalance=0;
	...
	public static boolean orderAgain(Scanner input){
		...
		if(orderAgain == 'Y' || orderAgain == 'y')
			return true;//continue from the start of the loop
		 	...
		 	MainCourseStarterBalance = MainCourseStarterBalance + qtyStarter;
